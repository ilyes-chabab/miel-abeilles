...
def analyze_parameters(population_rates, mutation_intensities, mutate_rates, generations):
    results = []
    total_combinations = len(population_rates) * len(mutation_intensities) * len(mutate_rates)
    
    for i, (population_rate, mutation_intensity, mutate_rate) in enumerate(itertools.product(population_rates, mutation_intensities, mutate_rates)):
        start_time = time.time()
        distance = run_simulation(population_rate, mutation_intensity, mutate_rate, generations)
        execution_time = time.time() - start_time
        results.append((population_rate, mutation_intensity, mutate_rate, distance, execution_time))
