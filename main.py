...
from hive import Hive, BEEHIVE_POS, POPULATION_SIZE, MUTATE_RATE
...
def plot_best_path(best_bee, BEEHIVE_POS, generation, best_distance, total_mutations, total_bees_generated): # Plot the best path found by the bees 
...
def main(num_generations): # Main function to run the genetic algorithm 
    ...
    while generation < num_generations: # Evolve the population for a specified number of generations
        hive.select_and_breed()
        generation += 1
        if hive.best_bee.distance < best_distance:
            best_distance = hive.best_bee.distance
        best_distances.append(hive.best_bee.distance)  # Ajout de la meilleure distance à la liste
    ...
    plot_best_path(hive.best_bee, BEEHIVE_POS, generation, best_distance, hive.total_mutations, hive.total_bees_generated)
